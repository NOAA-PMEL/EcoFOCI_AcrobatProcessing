...
if args.Instrument in ['GPS','gps']:
	rawdata = ACROBAT_data_read.get_inst_data(args.DataPath, source='gps', time_correction_seconds=30)
	print rawdata.resample(args.averaging_string,label='right',closed='right').mean().to_csv()
elif args.Instrument in ['fastcat','FastCAT']:
	rawdata = ACROBAT_data_read.get_inst_data(args.DataPath, source='fastcat', time_correction_seconds=0)
	print rawdata.resample(args.averaging_string,label='right',closed='right').mean().to_csv()
elif args.Instrument in ['TSG']:
	rawdata = ACROBAT_data_read.get_inst_data(args.DataPath, source='tsg', time_correction_seconds=30)
	print rawdata.resample(args.averaging_string,label='right',closed='right').mean().to_csv()
elif args.Instrument in ['ECOTriplet']:
	rawdata = ACROBAT_data_read.get_inst_data(args.DataPath, source='ECOTriplet', time_correction_seconds=0)
	print rawdata.resample(args.averaging_string,label='right',closed='right').mean().to_csv()
elif args.Instrument in ['ECO']:
	rawdata = ACROBAT_data_read.get_inst_data(args.DataPath, source='ECO', time_correction_seconds=30)
	print rawdata.resample(args.averaging_string,label='right',closed='right').mean().to_csv()
elif args.Instrument in ['ACROBAT','acrobat']:
	rawdata = ACROBAT_data_read.get_inst_data(args.DataPath, source='acrobat', , time_correction_seconds=30, UTC_offset_corr=7)
	print rawdata.resample(args.averaging_string,label='right',closed='right').mean().to_csv()
elif args.Instrument in ['AanOptode','optode']:
	rawdata = ACROBAT_data_read.get_inst_data(args.DataPath, source='optode', time_correction_seconds=0)
	print rawdata.resample(args.averaging_string,label='right',closed='right').mean().to_csv()
else:
